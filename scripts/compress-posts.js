...
async function compressSpecificFiles(directory, filePattern, options = {}) {
    console.log(`\n${'='.repeat(60)}`);
    console.log(`Compressing images matching '${filePattern}' in: ${directory}`);
    console.log(`${'='.repeat(60)}\n`);

    const backupDir = path.join(directory, 'post_images_original');

    // Create backup directory for post images
    try {
        await fs.access(backupDir);
        console.log(`⚠️  Backup directory already exists: ${backupDir}`);
        console.log(`⚠️  Skipping backup creation\n`);
    } catch {
        console.log(`Creating backup: ${backupDir}\n`);
        await fs.mkdir(backupDir, { recursive: true });

        // Copy original post images to backup
        const files = await fs.readdir(directory);
        const postFiles = files.filter(file => file.startsWith('post'));
    ...
    // Get post image files
    const files = await fs.readdir(directory);
    const imageFiles = files.filter(file => {
        const ext = path.extname(file).toLowerCase();
        return file.startsWith(filePattern) && ['.jpg', '.jpeg', '.png', '.webp'].includes(ext);
    });
    ...
    // Summary
    console.log(`${'='.repeat(60)}`);
    console.log(`Summary for post images:`);
    console.log(`  Original size: ${(totalOriginal / 1024 / 1024).toFixed(2)} MB`);
    console.log(`  Compressed size: ${(totalCompressed / 1024 / 1024).toFixed(2)} MB`);
    console.log(`  Total reduction: ${((1 - totalCompressed / totalOriginal) * 100).toFixed(1)}%`);
    console.log(`${'='.repeat(60)}\n`);
...
async function main() {
    ...
    try {
        // Compress Instagram post images
        await compressSpecificFiles(
            path.join(projectRoot, 'public', 'images'),
            'post',
            { quality: 82 }
        );

        console.log('\n✅ Instagram post images compressed successfully!\n');
        console.log('Original images backed up to public/images/post_images_original');
        console.log('You can delete the backup directory once you verify the compressed images look good.\n');
